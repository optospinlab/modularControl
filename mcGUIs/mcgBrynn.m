    ...
    methods (Static)
        ...
        function config = diamondConfig()
            ...
            config.controls = { { 'push',   'Open mcData',      'open',     'Operates the same way as the open button on an mcDataViewer window...' },...
                                ...
                                { 'edit',   'Below surf (um): ', 15,         'Focuses to surface, then moves down this number of um.',},...
        ...
        function Callbacks(gui, ~, ~, cbName)
            ...
            switch lower(cbName)
                ...
                case 'lmscan' % Added by Kelsey: large scan
                    ...
                    % Sets up optz input
                    configOptz = mcData.optimizeMoveConfig( gui.objects.piezo,...
                                                            gui.objects.counter,...
                                                            gui.controls{5}.Value,...
                                                            gui.controls{6}.Value,...
                                                            gui.controls{7}.Value,...
                                                            gui.controls{12}.Value); % axis, input, range, pixels, seconds
                    ...
                    configGalvo = mcData.squareScanConfig(  gui.objects.galvos(1),...
                                                            ...
                                                            gui.controls{3}.Value,...
                                                            gui.controls{2}.Value);
                    configGalvo.axes = {configGalvo.axes{1} configGalvo.axes{2} gui.objects.hwp}; % Append hwp to galvo arrays
                    configGalvo.scans = [configGalvo.scans hwpPos];
