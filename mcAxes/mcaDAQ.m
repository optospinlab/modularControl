...
%             config.dev =                'cDAQ1Mod1';
        ...
        function config = galvoXBrynnConfig()
            ...
            config.kind.int2extConv =   @(x)((x + 2.6253).*(-168.494));        % Conversion from 'internal' units to 'external'.
            config.kind.ext2intConv =   @(x)((x ./ (-168.494)) - 2.6253);      % Conversion from 'external' units to 'internal'.
            config.kind.intUnits =      'V';                    % 'Internal' units.
            config.kind.extUnits =      'um';                   % 'External' units.
...
%             config.kind.base =          -2.610;                 % The (internal) value that the axis seeks at startup.
        ...
        function config = galvoYBrynnConfig()
            ...
            config.kind.int2extConv =   @(x)((x + 0.5113).*(168.494));        % Conversion from 'internal' units to 'external'.
            config.kind.ext2intConv =   @(x)((x ./ (168.494)) - 0.5113);      % Conversion from 'external' units to 'internal'.
            config.kind.intUnits =      'V';                    % 'Internal' units.
            config.kind.extUnits =      'um';                   % 'External' units.
