...
% 2) Next, replace all of the lines starred with '**' with code appropriate to the new type.
%
% Keep in mind the separation between behavior (the content of the methods) and identity (the content of a.config).
%
% There are five (relevant) properties that are pre-defined in mcAxis that the user shoudl be aware of:
...
%       a.t         % An additional 'timer' session for unusal axes (see mcaMicro for use to poll the micros about the current position).
...
%
%   tf =    a.read()                            % Reads the current position of the axis, returns success. This is useful for 'slow' axes like micrometers.
%
...
%   x =     a.getXt(x)                          % Returns the target position of the axis (a.xt) in external units.
    ...
    methods (Static)    % Static configs
        ...
        function config = defaultConfig()               % Static config that should be used if no configuration is provided upon intialization.
            config = mcaTemplate.customConfig();        % ** Rename this to whatever static config should be used.
        end
        function config = customConfig()                % ** Use a descriptive name for this particular 'identity' of mca<MyNewAxis>.
            ...
            config.kind.intRange =      [-42 42];       % ** Change this to the range of the axis (e.g. [0 10] for a 0 -> 10 V DAQ piezo). This is in internal units (.extRange is generated by mcAxis).
    ...
    methods             % Initialization method
        function a = mcaTemplate(varin)                     % ** Insert mca<MyNewAxis> name here...
            ...
            if nargin == 0
                a.construct(mcaTemplate.defaultConfig());   % ** Insert mca<MyNewAxis> name here...
            else
                a.construct(varin);
            end
    ...
    % These methods overwrite the empty methods defined in mcAxis. These methods are used in the uncapitalized more-complex counterpart methods defined in mcAxis.
    methods
        ...
        function Open(a)                % Do whatever neccessary to initialize the axis.
            open(a.config.customVar1, a.config.customVar2);     % ** Change this to the custom code which opens the axis. Keep in mind that a.s should be used to store session info (e.g. serial ports, DAQ sessions).
        end
    ...
    methods
        ...
        function specificFunction(a)    % ** Rename to a descriptive name for the additional functionality.
            specific(a.x);              % ** Change to the appropriate code for this additional functionality.
        end
    end
end
