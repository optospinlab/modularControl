...
%             set(jButton,'ToolTipText', axis_.name());
            ...
            if      iscell(axis_.config.kind.intRange) && length(axis_.config.kind.intRange) == 2 &&...
                    xor([~axis_.config.kind.intRange{0} ~axis_.config.kind.intRange{1}], [~axis_.config.kind.intRange{1} ~axis_.config.kind.intRange{0}])    % If we should make a toggle switch instead of an edit box...
